...
ifeq ($(HAS_XDEBUG), 0)
	SYMFONY_PHPUNIT_VERSION=8.5.0 SYMFONY_PHPUNIT_REMOVE=symfony/yaml vendor/bin/simple-phpunit --dump-xdebug-filter $@
endif

test: build/xdebug-filter.php
ifeq ($(HAS_XDEBUG), 0)
	SYMFONY_PHPUNIT_VERSION=8.5.0 SYMFONY_PHPUNIT_REMOVE=symfony/yaml vendor/bin/simple-phpunit --prepend build/xdebug-filter.php -c phpunit.xml.dist --coverage-clover build/logs/clover.xml
else
	SYMFONY_PHPUNIT_VERSION=8.5.0 SYMFONY_PHPUNIT_REMOVE=symfony/yaml vendor/bin/simple-phpunit -c phpunit.xml.dist
endif
.PHONY: test
